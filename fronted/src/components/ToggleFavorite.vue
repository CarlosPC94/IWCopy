...
export default {
  ...
  created(){
    ...
    if(this.fav == null){
      if(this.wishlistProducts.findIndex(element=>element.id==this.products.id) != -1)
        this.favorited = true;
      else 
  ...
  methods: {
    quitarFav(index){
        ProductService.getOffProduct(index.id)      
    },
    añadirFav(product){
        ProductService.addProductWishList(product.id)
    },
    toggle() {
      // Only animate on favoriting.
      if (!this.favorited) {
        this.animating = true;
        this.añadirFav(this.products)
      } else {
        this.quitarFav(this.products)
      }
